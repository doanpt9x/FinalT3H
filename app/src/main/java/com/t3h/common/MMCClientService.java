...
import com.t3h.final_t3h.MessageActivity;
import com.t3h.final_t3h.MessagesDBManager;
 ...
 */
public class MMCClientService extends Service implements SinchClientListener {
    private static final String TAG = "MMCClientService";
    ...
    private MMCBroadcast mMCBroadcast = null;
    private String outGoingId = null, senderName = null;

    private MessageListener messageListener;
    private MessageClient messageClient;
    private MessagesDBManager messagesLogDBManager = null;
    ...
    public void onCreate() {
        ...
        if (messagesLogDBManager == null) {
            messagesLogDBManager = new MessagesDBManager(this);
        }
    ...
    private class OutGoingCallListener implements CallListener {
        ...
        public void onCallEstablished(Call call) {
            ...
            MMCClientService.this.sendBroadcast(intentPickUp);
        }
        ...
        public void onCallEnded(Call call) {
            ...
            MMCClientService.this.sendBroadcast(intentEndCall);
        }
    ...
    private class InComingCallListener implements CallClientListener, CallListener {
        ...
        public void onCallEstablished(Call call) {
            ...
            MMCClientService.this.sendBroadcast(intentAnswer);
        }
    ...
    private class MessageListener implements MessageClientListener {
        ...
        public void onIncomingMessage(final MessageClient messageClient, final Message message) {
            ...
            if (message.getHeaders().get("MAP") != null) {
                ...
                if (body.equals("MAP_OK")) {
                    ...
                    if (mapIntent.resolveActivity(MMCClientService.this.getPackageManager()) != null) {
                        MMCClientService.this.startActivity(mapIntent);
                    }
                    return;
                }
                final AlertDialog alertDialog = new AlertDialog.Builder(MMCClientService.this).create();
                ...
                alertDialog.setButton(AlertDialog.BUTTON_POSITIVE, "YES",
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                if (ContextCompat.checkSelfPermission(MMCClientService.this,
                                        Manifest.permission.ACCESS_FINE_LOCATION)
                                        != PackageManager.PERMISSION_GRANTED
                                        && ContextCompat.checkSelfPermission(MMCClientService.this,
            ...
            ActivityManager activityManager = (ActivityManager) MMCClientService.this
            ...
            if (GlobalApplication.startActivityMessage) {
                ...
                intentSent.putExtra(CommonValue.MMC_KEY_DATE, message.getHeaders().get("date"));
                intentSent.putExtra(CommonValue.MMC_KEY, key);
                intentSent.putExtra(CommonValue.MESSAGE_CONTENT, content);
                MMCClientService.this.sendBroadcast(intentSent);
            }
            ...
            if (tasks.get(0).processName.equals(CommonValue.PACKAGE_NAME_MAIN)) {
                ...
                if (!open) {
                    open = true;
                    MMCClientService.this.openChatHead(message.getSenderId(),
                            message.getHeaders().get("senderName"), content, date);
                }
            }

            MMCClientService.this.updateMessagesLogDBManager(id, fullName, content, date, isRead);
            if (!GlobalApplication.startWaitingMMC) {
                ...
                MMCClientService.this.sendBroadcast(intent);
            }
            if (GlobalApplication.checkLoginThisId) {
                ...
                MMCClientService.this.sendBroadcast(intentIncoming);
            }
        }
        ...
        public void onMessageSent(MessageClient messageClient, final Message message, String s) {
            if (message.getHeaders().get("MAP") != null) {
                ...
                if (GlobalApplication.startActivityMessage) {
                    ...
                    intentSent.putExtra(CommonValue.MMC_KEY_DATE, date);
                    intentSent.putExtra(CommonValue.MMC_KEY, key);
                    intentSent.putExtra(CommonValue.MESSAGE_CONTENT, content);
                    MMCClientService.this.sendBroadcast(intentSent);
                }
                ...
                MMCClientService.this.updateMessagesLogDBManager(id, fullName, "You:  " + content, date, 1);
                if (!GlobalApplication.startWaitingMMC) {
                    GlobalApplication.startWaitingMMC = true;
                    GlobalApplication.checkLoginThisId = false;
                    Intent intent = new Intent();
                ...
                if (GlobalApplication.checkLoginThisId) {
                    ...
                    MMCClientService.this.sendBroadcast(intentMessage);
                }
            }
        }
        ...
        public void onMessageDelivered(MessageClient messageClient, MessageDeliveryInfo messageDeliveryInfo) {
            ...
            MMCClientService.this.sendBroadcast(intentDelivered);
        }
    ...
    private void registerBroadcast() {
        if (mMCBroadcast == null) {
            mMCBroadcast = new MMCBroadcast();
            ...
            context.registerReceiver(mMCBroadcast, intentFilter);
        }
    }

    private class MMCBroadcast extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
            switch (intent.getAction()) {
                ...
                case CommonValue.ACTION_SEND_MESSAGE:
                    ...
                    date = intent.getStringExtra(CommonValue.MMC_KEY_DATE);
                    if (intent.getStringExtra(CommonValue.MMC_KEY) == null) {
                        MMCClientService.this.sendMessage(id, content, fullName, date);
                    } else {
                        switch (intent.getStringExtra(CommonValue.MMC_KEY)) {
                            case CommonValue.MMC_KEY_EMOTICON:
                                content = CommonValue.MMC_KEY_EMOTICON + content;
                                MMCClientService.this.sendMessage(id, content, fullName, date);
                                break;
                            case CommonValue.MMC_KEY_FILE:
                                MMCClientService.this.sendFile(id, content, fullName, date);
                                break;
                            case CommonValue.MMC_KEY_PICTURE:
                                MMCClientService.this.sendPicture(id, content, fullName, date);
                                break;
                        }
                    }
                    break;
            ...
            public void run() {
                ...
                try {
                    ...
                    parseFile.saveInBackground(new SaveCallback() {
                        ...
                        public void done(ParseException e) {
                            if (e != null) {
                                Toast.makeText(MMCClientService.this, "Missing send file",
                            ...
                            parseObject.saveInBackground(new SaveCallback() {
                                ...
                                public void done(ParseException e) {
                                    if (e != null) {
                                        ...
                                        Toast.makeText(MMCClientService.this, "Error send file",
                                                Toast.LENGTH_SHORT).show();
                                        return;
                                    }
                                    parseObject.put("senderId", outGoingId);
                                    parseObject.put("receiverId", id);
                                    final String content = CommonValue.MMC_KEY_FILE +
    ...
    private synchronized void sendPicture(final String id, final String pathPicture,
        ...
        if (!isChangeByte) {
            Toast.makeText(MMCClientService.this, "Send image not success!!!", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        parseFile.saveInBackground(new SaveCallback() {
            ...
            public void done(ParseException e) {
                ...
                parseObject.put("senderId", outGoingId);
                parseObject.put("receiverId", id);
                final String content = CommonValue.MMC_KEY_PICTURE + parseFile.getUrl();
                ...
                parseObject.saveInBackground(new SaveCallback() {
                    ...
                    public void done(ParseException e) {
                        if (e != null) {
                            Toast.makeText(MMCClientService.this, "Send image not success!!!",
                                    Toast.LENGTH_SHORT).show();
                        ...
                        message.addHeader("fullName", fullName);
                        message.addHeader("senderName", senderName);
                        MMCClientService.this.messageClient.send(message);
                    }
                });
            }
        }, new ProgressCallback() {
    ...
    private void openChatHead(final String id, final String fullName, final String content, final String date) {
        ...
        int index = ((GlobalApplication) MMCClientService.this.getApplication()).getAllFriendItems()
                .indexOf(new AllFriendItem(id, 1));
        if (index > -1) {
            imgAvatar.setImageBitmap(((GlobalApplication) MMCClientService.this
        ...
        widget.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intentChat = new Intent(MMCClientService.this, MessageActivity.class);
                ...
                MMCClientService.this.startActivity(intentChat);

                new Handler().post(new Runnable() {
                    @Override
                    public void run() {
                        MMCClientService.this.updateMessagesLogDBManager(id, fullName, content, date, 1);
                        Intent intentIncoming = new Intent();
                        intentIncoming.setAction(CommonValue.UPDATE_MESSAGE_LOG);
                        intentIncoming.putExtra(CommonValue.MESSAGE_LOG_ID, id);
                        intentIncoming.putExtra(CommonValue.MESSAGE_LOG_FULL_NAME, fullName);
    ...
    public void onDestroy() {
        ...
        this.unregisterReceiver(mMCBroadcast);
        super.onDestroy();
    }
}